...
| 5 | **Популярность каталога = `likes._count` (сырой счётчик лайков, без поправки на возраст)** | Prisma поддерживает `orderBy` по количеству связанных записей (`likes: { _count: 'desc' }`, TreeLike с `@@index([treeId])`); лайк — честная метрика вовлечённости; деревья без лайков откатываются к `createdAt` (tiebreaker) | старые деревья монополизируют топ, новые не пробиваются наверх | взвешенная формула рейтинга (лайки с затуханием по времени) или денормализованный счётчик в Tree |
