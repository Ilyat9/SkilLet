...
<p align="left">
  ...
  <a href="https://skil-let.vercel.app"><img src="https://img.shields.io/badge/live-skil--let.vercel.app-8c3b2e" alt="Live on Vercel" /></a>
...
**живое приложение: <https://skil-let.vercel.app>**
...
- Три способа создания дерева: вручную, из шаблона (Frontend / Backend / Data Science-ML / Иностранный язык — импорт одной транзакцией), AI-генерация по теме и сроку обучения через OpenAI-совместимый API — материал обязателен на каждом узле, число узлов растёт с выбранным сроком (без ключа фича предсказуемо недоступна)
- Экспорт дерева в JSON и импорт из файла с zod-валидацией и лимитами размера
- Сообщество: лайки (оптимистичный UI с откатом), комментарии с модерацией автором дерева, форки публичных деревьев с атрибуцией оригинала, переключение дерева между публичным и приватным, генеративные аватары профиля (DiceBear, тёмные наборы в стилистике темы)
